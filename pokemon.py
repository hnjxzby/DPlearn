...
def load_pokemon(root, mode='train'):
    ...
    # 读取Label信息
    # [file1,file2,], [3,1]
        ##这个是我均衡处理之后的数据集，比较大一点
    #images, labels = load_csv(root, 'imagesResample.csv', name2label)
        ####这个是我没有均衡处理的数据集
    images, labels = load_csv(root, 'images.csv', name2label)
    
###imagesResample.csv是均衡后的数据集，没有均衡前的数据集是images.csv
###hnjxzby@2021-10-11 mon
...
# 我觉得这里有问题，应该是用本次实验的数据图集计算出它们？hnjxzby@2021-10-2
...
def main():
    ...
    for step, (x,y) in enumerate(db):
        ...
        with writter.as_default():
            ...
            # 写入图片数据
            tf.summary.image('img',x,step=step,max_outputs=8)
           # time.sleep(5)
